...
#define URL_EVENTOS @"http://52.27.16.14/cut/eventos/ver"

@implementation EventosModel
...
/**
 * Este método llena la fuente de datos de los eventos
 *
 * Parametros
 * tabla:  Es el valor por referencia de la tabla de los eventos, se usa para que se
 * actualicen los eventos apenas se llena la fuente de los datos
 *
       ...
       parameters:parametros
          success:^(AFHTTPRequestOperation *operation, id responseObject) {
              ...
              if([[datos objectForKey:@"status"] isEqualToString:@"OK"]){
                  NSDictionary * items = [datos objectForKey:@"eventos"];
                  ...
                  if(_offset == 0){
                      [_mensajes errorConexion:@"Ocurrió un error cargando los eventos de la CUT." inView:view withLabel:_lblError];
                  }
              }else{
                  if(_offset == 0){
                      [_mensajes errorConexion:@"Ocurrió un error cargando los eventos de la CUT." inView:view withLabel:_lblError];
 ...
 * el inicio y se llama al método populateDataSource para cargar los eventos actuales
 *
 * Parametros
 * tabla:  Es el valor por referencia de la tabla delos eventos, se usa para que se
 * actualicen los eventos apenas se llena la fuente de los datos
 *
 * view: Es el valor por referencia de la vista de la tabla, se usa para mostrar los errores que
 * puedan aparecer o para mostrar la vista de cargando...
